...
export const getAnalytics = query({
    ...
    returns: v.object({
        ...
        chatAnalytics: v.object({
            ...
            chatsByModel: v.array(v.object({
                model: v.string(),
                count: v.number(),
                percentage: v.number(),
            })),
            chatCreationTrend: v.array(v.object({
                period: v.string(),
                count: v.number(),
                timestamp: v.number(),
            })),
            chatActivityHeatmap: v.array(v.object({
                hour: v.number(),
                day: v.number(),
                activity: v.number(),
            })),
        ...
        projectAnalytics: v.object({
            ...
            mostActiveProject: v.optional(v.object({
                name: v.string(),
                chatCount: v.number(),
                messageCount: v.number(),
            })),
            projectDistribution: v.array(v.object({
        ...
        libraryAnalytics: v.object({
            ...
            artifacts: v.object({
                total: v.number(),
                favorites: v.number(),
                byLanguage: v.array(v.object({
                    language: v.string(),
                    count: v.number(),
                })),
                mostReferenced: v.array(v.object({
                    filename: v.string(),
        ...
        timeAnalytics: v.object({
            activityByHour: v.array(v.object({
                hour: v.number(),
                activity: v.number(),
            })),
            activityByDay: v.array(v.object({
                day: v.string(),
                activity: v.number(),
            })),
    ...
    handler: async (ctx, args) => {
        ...
        const chats = await ctx.db
            ...
            .withIndex("by_user_updated", (q) => 
                q.eq("userId", userId).gte("updatedAt", start).lte("updatedAt", end)
            )
            .collect();
        ...
        const [attachments, artifacts, media] = await Promise.all([
            ctx.db.query("attachmentLibrary").withIndex("by_user", (q) => q.eq("userId", userId)).collect(),
            ctx.db.query("artifacts").withIndex("by_user", (q) => q.eq("userId", userId)).collect(),
            ctx.db.query("mediaLibrary").withIndex("by_user", (q) => q.eq("userId", userId)).collect(),
        ]);
        ...
        const overview = {
            ...
            averageMessagesPerChat: allChats.length > 0 ? Math.round(allMessages.length / allChats.length * 100) / 100 : 0,
            mostActiveDay: getMostActiveDay(messages),
            mostActiveHour: getMostActiveHour(messages),
        };

        // Calculate chat analytics
        const chatAnalytics = calculateChatAnalytics(chats, allChats, args.timeRange);

        // Calculate message analytics
        const messageAnalytics = calculateMessageAnalytics(messages, allMessages);

        // Calculate project analytics
        const projectAnalytics = calculateProjectAnalytics(projects, allChats, allMessages);

        // Calculate library analytics
        const libraryAnalytics = calculateLibraryAnalytics(attachments, artifacts, media);

        // Calculate collaboration analytics
        const collaborationAnalytics = calculateCollaborationAnalytics(allChats, projects);
...
export const getAnalyticsOverview = query({
    ...
    handler: async (ctx) => {
        ...
        const [allChats, allProjects, attachments, artifacts, media] = await Promise.all([
            ctx.db.query("chats").withIndex("by_user", (q) => q.eq("userId", userId)).collect(),
        ...
        const thisWeekChats = allChats.filter(chat => chat.createdAt >= oneWeekAgo).length;
        const thisWeekMessages = allMessages.filter(msg => msg.timestamp >= oneWeekAgo).length;
        ...
        return {
            ...
            totalLibraryItems: attachments.length + artifacts.length + media.length,
            thisWeekChats,
            thisWeekMessages,
            mostUsedModel,
...
function getMostActiveHour(messages: any[]): number {
    ...
    return Array.from(hourActivity.entries())
...
function calculateChatAnalytics(chats: any[], allChats: any[], timeRange: any) {
    ...
    const modelArray = Array.from(chatsByModel.entries()).map(([model, count]) => ({
        model,
        count,
    ...
    const periods = generatePeriods(timeRange.start, timeRange.end, timeRange.granularity);
    ...
    return {
        ...
        passwordProtectedChats: allChats.filter(c => c.isPasswordProtected).length,
...
function calculateMessageAnalytics(messages: any[], allMessages: any[]) {
    ...
    allMessages.forEach((msg, index) => {
        ...
        if (index > 0 && msg.role === "assistant" && allMessages[index - 1].role === "user") {
            responseTimes.push(msg.timestamp - allMessages[index - 1].timestamp);
        }
    });
    ...
    const multiAIMessages = allMessages.filter(m => m.metadata?.multiAIResponses);
    const averageModelsPerMessage = multiAIMessages.length > 0
        ? multiAIMessages.reduce((sum, msg) => sum + (msg.metadata?.multiAIResponses?.selectedModels?.length || 0), 0) / multiAIMessages.length
        : 0;
    ...
    return {
        ...
        longestMessage: Math.max(...allMessages.map(m => m.content.length), 0),
        commandUsage: Array.from(commandUsage.entries()).map(([command, count]) => ({ command, count })),
        modelUsage: Array.from(modelUsage.entries()).map(([model, count]) => ({
            model,
            count,
        ...
        responseTimeAnalytics: {
            averageResponseTime: responseTimes.length > 0 ? Math.round(responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length / 1000) : 0,
        ...
        multiAIUsage: {
            ...
            averageModelsPerMessage: Math.round(averageModelsPerMessage * 100) / 100,
            mostUsedCombination: [], // TODO: Calculate most used model combinations
        },
    };
}

function calculateProjectAnalytics(projects: any[], allChats: any[], allMessages: any[]) {
    const projectStats = projects.map(project => {
        const projectChats = allChats.filter(c => c.projectId === project._id);
        const projectMessages = projectChats.reduce((count, chat) => {
            return count + allMessages.filter(m => {
                // Check if message belongs to this chat's active branch
                return allChats.some(c => c._id === chat._id && c.activeBranchId && 
                    allMessages.some(msg => msg.branchId === c.activeBranchId));
            }).length;
        }, 0);
    ...
    const mostActive = projectStats.sort((a, b) => b.messageCount - a.messageCount)[0];

    return {
        totalProjects: projects.length,
        nestedProjects: projects.filter(p => p.parentId).length,
        averageChatsPerProject: projects.length > 0 ? Math.round(allChats.filter(c => c.projectId).length / projects.length * 100) / 100 : 0,
...
function calculateLibraryAnalytics(attachments: any[], artifacts: any[], media: any[]) {
    return {
        attachments: {
            ...
            mostUsed: attachments
                ...
                .slice(0, 5)
                .map(a => ({ name: a.originalName, usageCount: a.usageCount || 0 })),
        },
        artifacts: {
            ...
            mostReferenced: artifacts
                ...
                .slice(0, 5)
                .map(a => ({ filename: a.filename, referenceCount: a.referenceCount || 0 })),
        },
        media: {
            total: media.length,
...
function calculateCollaborationAnalytics(allChats: any[], projects: any[]) {
    ...
    const collaborativeChats = allChats.filter(c => c.shareMode === "collaboration").length;
    const totalViews = allChats.reduce((sum, c) => sum + (c.viewCount || 0), 0) +
    ...
    return {
        ...
        collaborativeChats,
        totalViews,
        averageViewsPerShare: (sharedChats + sharedProjects) > 0 ? Math.round(totalViews / (sharedChats + sharedProjects) * 100) / 100 : 0,
    };
}

function calculateTimeAnalytics(messages: any[]) {
    const activityByHour = new Array(24).fill(0).map((_, hour) => ({ hour, activity: 0 }));
    ...
    const peakHour = activityByHour.reduce((max, curr) => curr.activity > max.activity ? curr : max, activityByHour[0]);
    const peakDay = Array.from(activityByDay.entries()).reduce((max, curr) => curr[1] > max[1] ? curr : max, ["Monday", 0]);

    return {
        activityByHour,
        activityByDay: Array.from(activityByDay.entries()).map(([day, activity]) => ({ day, activity })),
...
function calculateAverageChatLifespan(chats: any[]): number {
    ...
    const avgMilliseconds = lifespans.reduce((sum, lifespan) => sum + lifespan, 0) / lifespans.length;
...
function getLanguageDistribution(artifacts: any[]) {
    ...
    return Array.from(languages.entries()).map(([language, count]) => ({ language, count }));
}
