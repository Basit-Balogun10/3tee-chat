...
import { getDeviceInfo, generateDeviceId } from "../lib/utils";
import { toast } from "sonner";

export function AuthComponent() {
    const { signIn } = useAuthActions();
    const storeSessionMetadata = useMutation(api.sessions.storeSessionMetadata);
    ...
    // Store session metadata after successful login
    const handleSessionMetadata = async () => {
        try {
            ...
            await storeSessionMetadata({
                ...
                deviceName: deviceInfo, // User-friendly name
                ipAddress: undefined, // Will be collected server-side if needed
            });

            console.log("✅ Session metadata stored successfully");
        } catch (error) {
            // Don't block the user experience if metadata storage fails
    ...
    const handleOAuthSignIn = async (provider: "github" | "google") => {
        try {
            ...
            // Store session metadata after successful OAuth login
            await handleSessionMetadata();
        } catch (error) {
    ...
    const handleAnonymousSignIn = async () => {
        try {
            ...
            // Store session metadata for anonymous users too
            await handleSessionMetadata();
    ...
    const handleVerifyOTP = async () => {
        ...
        try {
            ...
            // Store session metadata after successful email verification
            await handleSessionMetadata();
    ...
    useEffect(() => {
        const handleSessionInvalidated = () => {
            // Clear any local state/caches when session is invalidated
            localStorage.removeItem("sessionVerified");
            sessionStorage.clear();

            toast.info(
                "Your session has been signed out from another device.",
                {
                    duration: 5000,
                }
            );
        ...
        // Listen for session invalidation events (if implemented by Convex Auth)
        document.addEventListener(
            "convex:sessionInvalidated",
